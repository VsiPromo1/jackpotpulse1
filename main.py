...
def callback_check_subs(call):
    ...
    if check_subscriptions(user_id):
        ...
        bot.send_message(
    user_id,
    "<b>❗️ Ти відписався від наших спонсорських каналів.\nУсі бонуси скинуто. Підпишись знову, щоб грати!</b>",
    parse_mode='HTML'
)
...
def show_stats(message):
    ...
    cursor.execute("SELECT COUNT(*) FROM users WHERE lottery_participation = 1")
    participants_count = cursor.fetchone()[0]

    bot.send_message(
        message.chat.id,
        f"📊 Усього користувачів: {total}\n"
        f"🎟 У розіграші беруть участь: {participants_count}\n"
